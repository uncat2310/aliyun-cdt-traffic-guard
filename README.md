...
[![GHCR](https://img.shields.io/github/actions/workflow/status/uncat2310/aliyun-cdt-traffic-guard/docker-publish.yml?style=flat-square&label=GHCR)](https://github.com/uncat2310/aliyun-cdt-traffic-guard/actions)
...
服务部署在一台长期在线的机器上，通过阿里云 API 远程管理 ECS。不要装在会被关机的抢占式实例里。
...
控制台打开 [RAM 用户](https://ram.console.aliyun.com/users)，新建用户并开通 AccessKey。授权：

- `AliyunCDTFullAccess`
- `AliyunECSFullAccess`

保存 **AccessKey ID** 和 **AccessKey Secret**。到 [ECS 实例列表](https://ecs.console.aliyun.com/server/region/cn-hongkong) 复制实例 ID（形如 `i-j6c...`）和地域 ID（如 `cn-hongkong`）。
...
| `threshold_gb` | 月度安全额度，单位 GB。常用 `180` |
| `name` | 面板上显示的名称，可随便起 |

监控多台：在 `servers` 里再加一段，换一个 key（如 `hk-02`）和对应的实例 ID。每台可以用不同账号的 AccessKey。

只有一台时，`servers` 里只留一个节点。
...
浏览器打开 `http://服务器IP:8388`。

容器已设置 `restart: unless-stopped`，主机重启后会自动起来。
...
| 字段 | 含义 |
| --- | --- |
| `guard_enabled` | `true` 开启自动开关机 |
| `guard_auto_start` | `true` 表示低于阈值时自动开机 |
